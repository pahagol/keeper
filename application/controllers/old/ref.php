...
class Ref extends CI_Controller {
    ...
    function view($id) {
        $id = intval($id);
        $q = $this->db->get_where("p1_referals", array("id" => $id,"user_id"=>$this->user->id));
        if ($q->num_rows()) {
            $ref = $q->row();
            $d['referal'] = $ref;
        } else {
        ...
        $grid_js = build_grid_js('flex1', site_url("/ajax/ref_day_stats/$id"), $colModel, 'query_date', 'desc', $gridParams);
        $d['js_grid'] = $grid_js;
        $data['cont'] = $this->load->view("referal_view", $d, true);
